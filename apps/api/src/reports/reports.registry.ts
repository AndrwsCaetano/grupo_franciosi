...
export const PRODUCAO_MILHO_QUERY = `SELECT f.DESCRICAO_FILIAL filial,
       ...
       sum(vtr.SUB_TOTAL - vtr.des_umidade - vtr.des_impureza - vtr.des_avariado - vtr.des_ardido - vtr.des_outros - vtr.des_graos_verdes - vtr.des_quebrados) / 60 QTD_SC60,
       TS.QTDE_HA HECTARES,
...
FROM (
  SELECT f.DESCRICAO_FILIAL filial,
   ...
   WHERE g.SEQ_PLA_GRUPO IN (${GRUPOS_INSUMOS.map((g) => `'${g}'`).join(', ')})
     AND ef.SEQ_PLA_FAZENDA IS NOT NULL
     AND f2.DESC_FAZENDA IS NOT NULL
     AND TRIM(f2.DESC_FAZENDA) IS NOT NULL
) q
WHERE q.saldo <> 0
  AND q.rn = 1
  AND q.fazenda IS NOT NULL
  AND TRIM(q.fazenda) IS NOT NULL
ORDER BY 1, 3, 5, 6, 8`;
